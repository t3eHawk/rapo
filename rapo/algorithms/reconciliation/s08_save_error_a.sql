  ...
  from (
         select {parallelism} a.*,
          ...
          where {date_field_a} between to_date('{date_from:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS') and to_date('{date_to:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS')
            ...
            and (
                  ((o.correlation_type = 'O' or o.correlation_type is null) and (o.correlation_indicator is null or (o.correlation_indicator is not null and (m.discrepancy_time_a is not null {discrepancy_filters_a})))) or
                  (o.correlation_type = 'F' and o.correlation_indicator is not null and (m.discrepancy_time_a is not null {discrepancy_filters_a}))
                )
         ...
         select {parallelism} a.*,
                case
                     when o.correlation_indicator is not null and (m.discrepancy_time_a is not null {discrepancy_filters_a}) then 'Discrepancy'
                     when o.correlation_indicator is null and (m.discrepancy_time_a is not null {discrepancy_filters_a}) and {discrepancy_matching_exp} then 'Loss'
