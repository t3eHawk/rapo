create table rapo_temp_stage_a_{process_id} as
select {parallelism} a.*,
 ...
 where {date_field_a} between to_date('{date_from:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS') and to_date('{date_to:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS')
   ...
   and not exists (select e.{key_field_name_a} from rapo_temp_error_a_{process_id} e where {key_field_a} = e.{key_field_name_a})
