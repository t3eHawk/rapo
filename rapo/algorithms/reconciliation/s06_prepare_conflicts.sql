...
from (
  select m.a_id, m.b_id,
         row_number() over (partition by m.a_id order by m.distance_value_a, m.time_shift_rank_a, m.discrepancy_rank_a, m.b_id) as match_position_a,
         row_number() over (partition by m.b_id order by m.distance_value_b, m.time_shift_rank_b, m.discrepancy_rank_b, m.a_id) as match_position_b
