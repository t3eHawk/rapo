  ...
  from (
         select {parallelism} b.*,
          ...
          where {date_field_b} between to_date('{date_from:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS') and to_date('{date_to:%Y-%m-%d %H:%M:%S}', 'YYYY-MM-DD HH24:MI:SS')
            and (o.correlation_type in ('O', 'F') or o.correlation_type is null)
            and (
                  ((o.correlation_type = 'O' or o.correlation_type is null) and (o.correlation_indicator is null or (o.correlation_indicator is not null and (m.discrepancy_time_b is not null {discrepancy_filters_b})))) or
                  (o.correlation_type = 'F' and o.correlation_indicator is not null and (m.discrepancy_time_b is not null {discrepancy_filters_b}))
                )
         ...
         select {parallelism} b.*,
                case
                     when o.correlation_indicator is not null and (m.discrepancy_time_b is not null {discrepancy_filters_b}) then 'Discrepancy'
                     when o.correlation_indicator is null and (m.discrepancy_time_b is not null {discrepancy_filters_b}) and {discrepancy_matching_exp} then 'Loss'
                     when o.correlation_indicator is null then 'Duplicate'
                end as rapo_result_type,
                cast(m.b_id as varchar2(4000)) as rapo_discrepancy_id,
                case when m.discrepancy_time_b is not null then m.discrepancy_time_b||'|'||m.discrepancy_time_value||';' end{discrepancy_descriptions_b} as rapo_discrepancy_description
           from rapo_temp_source_b_{process_id} b left join rapo_temp_t02_org_b_{process_id} o on {key_field_b} = o.b_id
